...
* Messages send via `postOutgoingMessage` are processed by the IMA Agent, a containerized nodejs script operating on each SKALE Node and listening for events on the SKALE Chain and Mainnet. 
...
* Your SKALE Chain contract(s) must call `postOutgoingMessage("Mainnet", [RECEIVING_CONTRACT_ADDRESS_ON_MAINNET], [AMOUNT], [FINAL RECEIVER/CONTRACT ADDRESS], [DATA])`
...
* Your Mainnet contract(s) must call `postOutgoingMessage([SKALE_CHAIN_ID], [RECEIVING_CONTRACT_ADDRESS_ON_SCHAIN], [AMOUNT], [FINAL RECEIVER/CONTRACT ADDRESS_ON_SCHAIN], [DATA])`
* The receiving contract address on SKALE Chain must implement a `postMessage` function.

.To send only data

You only need to pass the [SKALE_CHAIN_ID] and [DATA] parameters and implement the postMessage logic to handle the data. The [RECEIVING_CONTRACT_ADDRESS_ON_SCHAIN], [FINAL RECEIVER/CONTRACT ADDRESS_ON_SCHAIN] and [AMOUNT] parameters can be ignored.
