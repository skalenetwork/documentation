...
Contract defines the SKALE token and is based on ERC777 token
implementation.
...
Allows SkaleManager to mint an amount of tokens and transfer minted
...
Returns and updates the delegated amount in SKL tokens for a given wallet.
...
Returns and updates the slashed amount in SKL tokens for a given wallet.
...
Returns and updates the locked amount in SKL tokens for a given wallet.
