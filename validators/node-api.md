...
> ❗️ WARNING: REST API provides access to private methods such as money transfers, node controls, and more. Make sure that port `3007` is available only to your trusted clients.
...
| `/api/v1/node/send-tg-notification` | `POST` | `ALLOWED`         | Send Telegram notification to the node owner. Telegram bot could be disabled by node owner. | `message` - Array of message strings                                                                  | N/A                                      |
