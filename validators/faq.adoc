...
You cannot change your node IP address, but what is possible is to remove the node with the incorrect/old IP address and register a new node under the correct/new IP while keeping the old SGX key.
...
To perform a node exit, keep the original node machine and its data and update the IP, follow these steps:
